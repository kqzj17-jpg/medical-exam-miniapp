...
assert(examWxml.indexOf('class="q-col"') !== -1, '题干/选项须放在主栏内居中的 q-col')
assert(examWxml.indexOf('grid-scroll') !== -1, '底栏答题卡须可纵向滚动，不得一次铺开全卷')
...
assert(examJs.indexOf('跨题型不可回看') !== -1, '锁定分段不可点选回看')
assert(examJs.indexOf('canEnterSection') !== -1 && examJs.indexOf('canGoTo') !== -1, '分段跳转与题号跳转须校验跨题型')
assert(examJs.indexOf('unansweredCount') !== -1, '离开本段若有未答须提示')
assert(ui.NAV_H <= 80 && ui.TITLE_H <= 28 && ui.TYPEBAR_H <= 28, '作答 chrome 须压缩，把高度让给题干')
...
assert(ui.SIDE_RATIO >= 0.18 && ui.SIDE_RATIO <= 0.2, '左栏比例须约 18–20%')
assert(ui.SIDE_MAX <= 200 && ui.SIDE_MAX >= 180, '左栏最大宽度约 180–200')
assert(ui.SIDE_GAP >= 12, '侧栏与主栏之间须留出间隙')
assert(/gap:\s*16px/.test(examWxss), 'workspace 须有侧栏与主栏间隙')
assert(/\.q-col\s*\{/.test(examWxss), '题干区须有 q-col 居中')
assert(/max-width:\s*720px/.test(examWxss), '题干/选项块须在主栏内限宽居中，避免贴侧栏、右侧大片空白')
assert(/margin-left:\s*auto/.test(examWxss) && /margin-right:\s*auto/.test(examWxss), 'q-col 须 margin auto 居中')
assert(/\.grid-scroll\s*\{/.test(examWxss), '答题卡须有限高滚动容器')
assert(/max-height:\s*32px/.test(examWxss), '答题卡滚动区须限高，避免挤占题干')
...
const results = sizes.map((s) => {
  ...
  assert(layout.sideW <= ui.SIDE_MAX, s.name + ' 左栏不得宽于 SIDE_MAX')
  assert(layout.sideW >= Math.min(140, layout.innerW * 0.2) - 1, s.name + ' 左栏过窄')
  assert(layout.sideW / layout.innerW <= 0.28, s.name + ' 左栏占比应约 18–20%')
  assert(layout.sideGap === ui.SIDE_GAP, s.name + ' 主栏与侧栏须计入间隙')
  assert(layout.mainW === layout.innerW - layout.sideW - layout.sideGap, s.name + ' 主栏宽度须扣除侧栏与间隙')
